...
void USER :: login(char* keyword="user") {
	...
	ifstream ifile("dduser.dat", ios::in|ios::binary);
	ofstream ofile("dduser.dat", ios::app|ios::binary);
	...
	if(option==8)
		...
		remove("dduser.dat");
		error_message("User file deleted.");
		goto start;
	}
		...
		gotoxy(40,14);
		if(option=='2') {
			strcpy(U1.password,get_pass("create"));
			ofile.write((char*)&U1, sizeof(USER));
			ofile.close();
			return;
		}
		else {
			...
			while(ifile.read((char*)&U1, sizeof(USER)))
				if(strcmpi(U1.ret_user(), temp_user)==0)
					if(strcmp(U1.ret_pass(), temp_pass)==0)
						bool=1;
		}
		if(option=='3') {
			strcpy(notes_file, U1.ret_user());
			strcpy(contacts_file, U1.ret_user());
			...
			ofile.seekp(loc+sizeof(USER), ios::beg);
			while(ifile.read((char*)&U1, sizeof(USER))) {
				ofile.seekp(-2*sizeof(USER), ios::cur);
				ofile.write((char*)&U1, sizeof(USER));
				ofile.seekp(sizeof(USER), ios::cur);
			}
...
float char_to_num (char p[]) {
	float num=0;
	int decimal=0, dp=-1;				//dp-decimal place
	for(; *p !='\0'; ++p) {
		if(*p=='.') {
			decimal=1;
			continue;
...
int calculate () {
	...
	float result= N_S.top();
	N_S.pop();
	return result;
}

void calculator() {
	float history[4], h_index=0;
	start:
	clrscr();
	border();

	if(h_index==4) {					//deletes the first history
		for(int i=0; i<3; ++i)
...
void horoscope() {
	...
	if(!infile) {
		error_message("Horoscope file not found.");
		return;
	}
